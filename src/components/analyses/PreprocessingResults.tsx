...
export default function PreprocessingResults({ qcReport }: PreprocessingResultsProps) {
  ...
  const read = `${qcReport.samples_passed} of ${qcReport.total_input_samples} samples passed QC · ${qcReport.genes_after_filter.toLocaleString()} of ${qcReport.genes_before_filter.toLocaleString()} genes retained (${filterPct}% filtered out).`;
